...
/*---- Global Variable Declarations and Def ----*/
struct eth_addr lidar_remote_mac;
extern struct netif server_netif;
static struct udp_pcb *pcb;
static struct perf_stats server;
uint8_t livox_comm_active = FALSE;  //For USB Handling and Heart Check
struct udp_pcb *lidar_response_pcb;	 /*This is to Send and receive the Commands as well as ack*/
struct udp_pcb *lidar_cmd_pcb;		/*This is to Receive the Point cloud data*/


uint16_t lidar_remote_port;
uint16_t gen_local_port = GEN_CMD_PORT;
ip_addr_t lidar_remote_ip;
ip_addr_t board_local_ip;
		...
		struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
	...
	xil_printf("Recv_ID: %u\r\n",recv_id);

	/*---- Copying the Address and the Port of the Sensor ----*/
	pcb->remote_ip = *addr;
	pcb->remote_port = port;
	lidar_remote_port = port;
	lidar_remote_ip = *addr;
	reset_stats();
	...
	uint8_t msgFF[p->len];
	memcpy(msgFF,(uint8_t *)p->payload,p->len);
	for(int i=0;i<(p->len);i++)
	{
		xil_printf("%02x ",msgFF[i]);
	}
	xil_printf("\r\n");


	if (msgFF[SOF_IDX] == 0xAA && (msgFF[CMD_SET_IDX] == 0X00 && msgFF[CMD_ID_IDX] == 0x00))
	{ // Check if it matches the broadcast structure
		xil_printf("Broadcast message received, processing...\r\n");
		uint16_t len = p->len;
		pbuf_free(p);
		// Call your custom handling function
		tp_HandleLivoxMsg(msgFF, len, tpcb);
	}
...
//	if (first && (recv_id == 0))
	if (first && (recv_id != 0))
	{
		...
		print_udp_conn_stats();
		first = 0;
		/* After storing the Remote ip and Remote Port we're Handling the Livox Msg*/
		if(msgFF[4] == 0x02)
		{
			if(msgFF[10] == 0x00)
			{
//				tp_HandleLivoxMsg(msgFF, p->len);
			}
		}
	}
	else if (first)
...
void start_application(void)
{
	...
	xil_printf("Entering into udp_recv\r\n");
